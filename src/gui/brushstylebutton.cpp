...
{
    for (auto it = brush_style_codes.keyBegin(); it != brush_style_codes.keyEnd(); ++it)
    {
...
void BrushStyleButton::toolChanged(const Tool *tool)
{
    if (tool && tool->tool() & Tool::AnyDrawTool)
    {
        const int index = findData(QVariant::fromValue(static_cast<const DrawTool*>(tool)->brush().style()));
        if (index >= 0)
