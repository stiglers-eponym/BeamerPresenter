...
ActionButton::ActionButton(ToolSelectorWidget *parent) : QToolButton(parent)
{
  ...
  connect(this, &ActionButton::sendAction, master(), &Master::handleAction,
          Qt::QueuedConnection);
}
    ...
    : ActionButton(parent)
{
  ...
  if (action_to_custom_icons(action).length() > 1)
    connect(parent, &ToolSelectorWidget::sendStatus, this,
            &ActionButton::setStatus);
}

void ActionButton::setStatus(const Action action, const int status)
{
  if (status >= 0 && actions.contains(action) &&
      status < action_to_custom_icons(action).length()) {
    display_action = action;
...
void ActionButton::updateIcon()
{
  ...
  if (display_status >= 0)
    setIcon(
        QIcon(preferences()->icon_path + "/actions/" +
              action_to_custom_icons(display_action).value(display_status)));
  else
    update();
}
...
{
  if (action == InvalidAction) return;
  ...
  switch (action) {
    ...
    case Update:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "update view (use if e.g. slide has bad resolution after resizing "
          "the window)");
    case NextPage:
    ...
    case NextSkippingOverlays:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "go to next page which has not the same page "
                               "label as the current page");
    case PreviousSkippingOverlays:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "go to previous page which has not the same page label as the "
          "current page");
    case FirstPage:
    ...
    case UndoDrawing:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "undo last drawing/erasing step in the "
                               "presentation on the current slide");
    case UndoDrawingLeft:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "undo last drawing/erasing step on the left half of a PDF "
          "document which is split into notes and a presentation");
    case UndoDrawingRight:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "undo last drawing/erasing step on the right half of a PDF "
          "document which is split into notes and a presentation");
    case RedoDrawing:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "redo undone drawing/erasing step in the presentation "
          "on the current slide");
    case RedoDrawingLeft:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "redo last drawing/erasing step on the left half of a PDF "
          "document which is split into notes and a presentation");
    case RedoDrawingRight:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "redo last drawing/erasing step on the right half of a PDF "
          "document which is split into notes and a presentation");
    case ClearDrawing:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "clear all drawings on the current presentation slide");
    case ClearDrawingLeft:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "clear all drawings of the current slide on the left half of "
          "a PDF document which is split into notes and a presentation");
    case ClearDrawingRight:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "clear all drawings of the current slide on the right half of "
          "a PDF document which is split into notes and a presentation");
    case ScrollDown:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "scroll down presentation view: adds extra space below "
          "the slide for drawing");
    case ScrollUp:
    ...
    case CutClipboard:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "remove selected items and copy them to the clipboard");
    case PasteClipboard:
      return QT_TRANSLATE_NOOP("ActionButton", "paste from clipboard");
    case SelectionToForeground:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "bring selected items to the foreground");
    case SelectionToBackground:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "bring selected items to the background");
    case RemoveSelectedItems:
    ...
    case StartTimer:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "start or resume presentation timer");
    case StopTimer:
      return QT_TRANSLATE_NOOP("ActionButton", "pause presentation timer");
    case StartStopTimer:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "pause or continue presentation timer");
    case ResetTimePassed:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "reset timer: sets passed time to zero");
    // Media
    case PlayMedia:
      return QT_TRANSLATE_NOOP("ActionButton",
    ...
    case PauseMedia:
      return QT_TRANSLATE_NOOP("ActionButton",
                               "pause all videos on the current slide");
    case PlayPauseMedia:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "If any video is playing: pause all videos. Otherwise: "
          "start all videos.");
    case Mute:
    ...
    case FullScreen:
      return QT_TRANSLATE_NOOP(
          "ActionButton", "toggle fullscreen mode for currently active window");
    case Quit:
      return QT_TRANSLATE_NOOP(
          "ActionButton",
          "quit and ask to save drawings if there are unsaved changes. "
          "Detection of unsaved changes is not reliable yet.");
    case QuitNoConfirmation:
      return QT_TRANSLATE_NOOP("ActionButton",
