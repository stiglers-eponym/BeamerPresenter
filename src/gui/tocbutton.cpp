...
TOCbutton::TOCbutton(const QString &title, const int _page,
                     QPushButton *expand_button, QWidget *parent)
    : QPushButton(title, parent), expand_button(expand_button), page(_page)
{
  ...
  if (expand_button) {
    ...
    connect(expand_button, &QPushButton::toggled, this,
            &TOCbutton::toggleVisibility);
    expand_button->setToolTip(tr("uncheck to hide child layers"));
  }
  connect(this, &QPushButton::clicked, this,
          [&] { emit sendNavigationEvent(page); });
...
void TOCbutton::expand()
{
  ...
  while (child) {
    child->show();
    if (child->expand_button) child->expand_button->show();
...
void TOCbutton::collapse()
{
  ...
  if (tree_child) tree_child->collapse_hide();
  parentWidget()->updateGeometry();
}
...
void TOCbutton::collapse_hide()
{
  ...
  if (tree_child) tree_child->collapse_hide();
  if (tree_next) tree_next->collapse_hide();
}
