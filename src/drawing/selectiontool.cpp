...
constexpr qreal RAD_TO_DEG = 180 / 3.1415926535897932384626433;
...
void SelectionTool::startRectSelection(const QPointF &pos,
...
void SelectionTool::startFreehandSelection(const QPointF &pos,
                                           const void *scene) noexcept
{
  _scene = scene;
...
void SelectionTool::startRotation(const QPointF &reference,
                                  const QPointF &center) noexcept
{
  changeType(Rotate);
  const QPointF vec = reference - center;
  properties =
      rotation_properties{center, RAD_TO_DEG * std::atan2(vec.y(), vec.x())};
}
  ...
  try {
    switch (_type) {
      ...
      case Move: {
        ...
        for (auto it = initial_transforms.cbegin();
             it != initial_transforms.cend(); ++it)
    ...
    qWarning() << "unhandled exception in SelectionTool::liveUpdate"
               << properties.index();
  }
}
...
void SelectionTool::setLiveRotation(const QPointF &pos)
{
  ...
  const qreal angle =
      RAD_TO_DEG * std::atan2(vec.y(), vec.x()) - prop.start_angle;
  QTransform rotation;
  rotation.rotate(angle);
  QTransform transform;
  QPointF new_origin;
  qreal dx, dy;
  for (auto it = initial_transforms.cbegin(); it != initial_transforms.cend();
       ++it) {
    ...
    new_origin = it.key()->mapFromScene(
        prop.rotation_center +
        (it.key()->scenePos() - prop.rotation_center) * rotation);
    // Translate to new origin.
    transform =
        QTransform().translate(new_origin.x(), new_origin.y()) * transform;
    it.key()->setTransform(transform);
  }
}
...
void SelectionTool::setLiveScaling(const QPointF &pos)
{
  ...
  QTransform scale;
  scale.scale(prop.reference.x() == prop.start_handle.x()
                  ? 1
                  : (pos.x() - prop.reference.x()) /
                        (prop.start_handle.x() - prop.reference.x()),
              prop.reference.y() == prop.start_handle.y()
                  ? 1
                  : (pos.y() - prop.reference.y()) /
                        (prop.start_handle.y() - prop.reference.y()));
  QTransform transform;
  QPointF new_origin;
  qreal dx, dy;
  for (auto it = initial_transforms.cbegin(); it != initial_transforms.cend();
    ...
    // First construct it in scene coordinates, then map to item coordinates.
    new_origin = it.key()->mapFromScene(
        prop.reference + (it.key()->scenePos() - prop.reference) * scale);
    // Translate to new origin.
    transform =
        QTransform().translate(new_origin.x(), new_origin.y()) * transform;
    it.key()->setTransform(transform);
...
void SelectionTool::startScaling(const QPointF &movable,
                                 const QPointF &fixed) noexcept
{
  changeType(Resize);
...
void SelectionTool::initTransformations(
    const QList<QGraphicsItem *> &items) noexcept
{
  initial_transforms.clear();
  for (const auto item : items) initial_transforms[item] = item->transform();
}
...
const QPolygonF SelectionTool::polygon() const noexcept
{
  try {
    ...
    if (_type == SelectPolygon)
      if (auto prop = std::get<QPolygonF *>(properties)) return *prop;
  } catch (...) {
  }
  return QPolygonF();
}
