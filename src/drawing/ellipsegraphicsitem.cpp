...
EllipseGraphicsItem::EllipseGraphicsItem(const DrawTool &tool,
                                         const QPointF &pos,
                                         QGraphicsItem *parent)
    : QGraphicsEllipseItem(pos.x(), pos.y(), 0, 0, parent), tool(tool)
{
...
BasicGraphicsPath *EllipseGraphicsItem::toPath() const
{
  ...
  const qreal rx = therect.width() / 2, ry = therect.height() / 2,
              rx_plus_pen = rx + pen().widthF() / 2,
              ry_plus_pen = ry + pen().widthF() / 2,
              phasestep = 2 * PI / segments, cx = therect.center().x(),
              cy = therect.center().y();
  ...
  BasicGraphicsPath *path = new BasicGraphicsPath(
      tool, coordinates,
      QRectF(-rx_plus_pen, -ry_plus_pen, 2 * rx_plus_pen, 2 * ry_plus_pen));
