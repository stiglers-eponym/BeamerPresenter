...
class ShapeRecognizer
{
  ...
  struct Moments {
    ...
    {
      return {s + other.s,     sx + other.sx,   sy + other.sy,
              sxx + other.sxx, sxy + other.sxy, syy + other.syy};
    }
    ...
    void reset() noexcept { s = sx = sy = sxx = sxy = syy = 0; }
    ...
    /// Compute standard deviation (square root of variance, normalized to
    /// weights).
   ...
   *
   * This returns
   * \[ \sum_k w_k \left[ \frac{(x_k-m_x)^2}{rx^2} + \frac{(y_k-m_y)^2}{ry^2} -
   * 1 \right]^2 \] where (x_k, y_k) is a coordinate with weight w_k.
   */
  qreal ellipseLossFunc(const qreal mx, const qreal my, const qreal ax,
                        const qreal ay) const noexcept
  {
    const qreal bc = mx * mx * ax + my * my * ay - 1;
    return moments.s * bc * bc + sxxxx * ax * ax + syyyy * ay * ay +
           2 * sxxyy * ax * ay +
   ...
   * @see ellipseLossFunc
   */
  qreal ellipseLossGradient_mx(const qreal mx, const qreal my, const qreal ax,
                               const qreal ay) const noexcept
  {
    const qreal bc = mx * mx * ax + my * my * ay - 1;
    return 4 * moments.s * bc * mx * ax + 12 * mx * moments.sxx * ax * ax +
           4 * mx * moments.syy * ax * ay + 8 * my * moments.sxy * ax * ay -
           4 * ax * (sxxx * ax + sxyy * ay) - 4 * bc * moments.sx * ax -
   ...
   * @see ellipseLossFunc
   */
  qreal ellipseLossGradient_my(const qreal mx, const qreal my, const qreal ax,
                               const qreal ay) const noexcept
  {
    const qreal bc = mx * mx * ax + my * my * ay - 1;
    return 4 * moments.s * bc * my * ay + 12 * my * moments.syy * ay * ay +
           4 * my * moments.sxx * ax * ay + 8 * mx * moments.sxy * ax * ay -
           4 * ay * (sxxy * ax + syyy * ay) - 4 * bc * moments.sy * ay -
   ...
   * @see ellipseLossFunc
   */
  qreal ellipseLossGradient_ax(const qreal mx, const qreal my, const qreal ax,
                               const qreal ay) const noexcept
  {
    const qreal bc = mx * mx * ax + my * my * ay - 1;
    return 2 * moments.s * mx * mx * bc + 2 * sxxxx * ax + 2 * sxxyy * ay +
           2 * (5 * mx * mx * ax + bc) * moments.sxx +
           2 * mx * mx * moments.syy * ay + 8 * mx * my * moments.sxy * ay -
           4 * mx * (2 * sxxx * ax + sxyy * ay) - 4 * my * ay * sxxy -
   ...
   * @see ellipseLossFunc
   */
  qreal ellipseLossGradient_ay(const qreal mx, const qreal my, const qreal ax,
                               const qreal ay) const noexcept
  {
    const qreal bc = mx * mx * ax + my * my * ay - 1;
    return 2 * moments.s * my * my * bc + 2 * syyyy * ay + 2 * sxxyy * ax +
           2 * (5 * my * my * ay + bc) * moments.syy +
           2 * my * my * moments.sxx * ax + 8 * mx * my * moments.sxy * ax -
           4 * my * (sxxy * ax + 2 * syyy * ay) - 4 * mx * ax * sxyy -
