...
struct DrawToolDifference {
  ...
  DrawToolDifference(const DrawTool &old_tool, const DrawTool &new_tool)
      : old_pen(old_tool.pen()),
        new_pen(new_tool.pen()),
        old_brush(old_tool.brush()),
        new_brush(new_tool.brush()),
        old_mode(old_tool.compositionMode()),
        new_mode(new_tool.compositionMode())
  {
  }
};
...
struct Step {
  ...
  // std::variant<ZValueChange,QTransform,DrawToolDifference,TextPropertiesDifference>>
  // changes;
  ...
  bool empty() const
  {
    return transformedItems.empty() && drawToolChanges.empty() &&
           textPropertiesChanges.empty() && createdItems.empty() &&
           deletedItems.empty() && z_value_changes.empty();
  }
};
}  // namespace drawHistory
...
/**
 * @brief Collection of QGraphicsItems including a history of changes to these
 * items
 *
...
class PathContainer : public QObject
{
 ...
 private:
   ...
   * * inHistory == -2 indicates that this entry has been created as a copy and
  ...
  void limitHistory()
  {
    ...
    if (history.length() > target_length) clearHistory(target_length);
  }
 ...
 public:
  ...
  struct VisibleIterator {
    ...
    using difference_type =
        QHash<QGraphicsItem *, LookUpProperties>::difference_type;
    using value_type = QGraphicsItem *;
    using pointer = QGraphicsItem *const *;
    using reference = QGraphicsItem *const &;
    VisibleIterator(
        const std::unordered_map<QGraphicsItem *,
                                 LookUpProperties>::const_iterator &it,
    ...
    {
      while (++_it != _map.cend() && !_it->second.visible) {
      };
      return *this;
    }
    VisibleIterator operator++(int)
    {
      VisibleIterator tmp = *this;
      ++(*this);
      return tmp;
    }
  ...
  VisibleIterator begin() const
  {
    ...
    if (_ref_count.empty()) return VisibleIterator(it, _ref_count);
  ...
  {
    if (_z_order.empty()) return 10;
    return (*_z_order.crbegin())->zValue();
  }
  ...
  {
    if (_z_order.empty()) return 0;
    return (*_z_order.cbegin())->zValue();
  }

  /// Create a new PathContainer which is a copy of this but does not have any
  /// history.
  ...
  /// Add a new QGraphisItem* in a new history step and bring it to the
  /// foreground.
  ...
  bool empty() const noexcept { return _ref_count.empty(); }
  ...
  bool isPlainCopy() const noexcept { return inHistory == -2; }
  ...
  /// @see loadDrawings(QXmlStreamReader &reader)
  /// @see writeXml(QXmlStreamWriter &writer) const
  static void loadDrawings(QXmlStreamReader &reader, PathContainer *center,
                           PathContainer *left, PathContainer *right,
  ...
  /// They can then inform this function, which removes them and adds this as a
  /// new history step.
 ...
 public slots:
  ...
  void removeItem(QGraphicsItem *item) { replaceItem(item, nullptr); }

  /// Notify of a change in a text item, add the item to history if necessary.
  void addTextItem(QGraphicsItem *item) { replaceItem(nullptr, item); }
};
