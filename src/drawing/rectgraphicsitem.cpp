...
RectGraphicsItem::RectGraphicsItem(const DrawTool &tool, const QPointF &pos,
                                   QGraphicsItem *parent)
    : QGraphicsRectItem(pos.x(), pos.y(), 0, 0, parent), tool(tool)
{
...
void RectGraphicsItem::setSecondPoint(const QPointF &pos)
{
  ...
  if (newrect.width() < 0) origin ^= OriginRight;
  if (newrect.height() < 0) origin ^= OriginBottom;
...
BasicGraphicsPath *RectGraphicsItem::toPath() const
{
  ...
  if (therect.isEmpty()) return nullptr;
  const int nx = therect.width() / 10 + 2, ny = therect.height() / 10 + 2;
  const qreal xscale = therect.width() / nx, yscale = therect.height() / ny,
              left = therect.left(), right = therect.right(),
              top = therect.top(), bottom = therect.bottom(),
              rleft = (left - right) / 2, rtop = (top - bottom) / 2;
  QVector<QPointF> coordinates(2 * (nx + ny) + 1);
  for (int i = 0; i < nx; ++i) coordinates[i] = {rleft + xscale * i, rtop};
  for (int i = 0; i < ny; ++i)
