...
{
  if (!shape_cache.isEmpty() || coordinates.isEmpty()) return shape_cache;
  debug_verbose(DebugDrawing, "calculating shape" << coordinates.length()
                                                  << this << bounding_rect);
  QPainterPath path;
  if (coordinates.length() == 1) {
...
void AbstractGraphicsPath::finalize()
{
  ...
  for (auto &point : coordinates) point = mapToScene(point) - new_scene_pos;
  resetTransform();
