...
FullGraphicsPath::FullGraphicsPath(const DrawTool &tool, const QPointF &pos,
...
FullGraphicsPath::FullGraphicsPath(const DrawTool &tool,
                                   const QVector<QPointF> &coordinates,
                                   const QVector<float> &pressures)
    : AbstractGraphicsPath(tool, coordinates), pressures(pressures)
{
  shape_cache = shape();
  bounding_rect = shape_cache.boundingRect();
}

FullGraphicsPath::FullGraphicsPath(const FullGraphicsPath *const other,
                                   int first, int last)
    : AbstractGraphicsPath(other->_tool)
{
  ...
  if (first < 0) first = 0;
  if (last > other->size() || last < 0) last = other->size();
  ...
  pressures = QVector<float>(other->pressures.cbegin() + first,
                             other->pressures.cbegin() + last);
  coordinates = QVector<QPointF>(other->coordinates.cbegin() + first,
                                 other->coordinates.cbegin() + last);
#else
    ...
    : AbstractGraphicsPath(tool)
{
  ...
  while (coordinate_list.length() > 1) {
    coordinates[i] = {coordinate_list.takeFirst().toDouble(),
                      coordinate_list.takeFirst().toDouble()};
...
void FullGraphicsPath::paint(QPainter *painter,
                             ...
                             QWidget *widget)
{
  ...
  if ((preferences()->debug_level & (DebugDrawing | DebugVerbose)) ==
      (DebugDrawing | DebugVerbose)) {
...
QList<AbstractGraphicsPath *> FullGraphicsPath::splitErase(
    const QPointF &scene_pos, const qreal size) const
{
  if (!sceneBoundingRect()
           .marginsAdded(QMarginsF(size, size, size, size))
           .contains(scene_pos))
  ...
  if (last > first + 2) list.append(new FullGraphicsPath(this, first, last));
  return list;
}
...
void FullGraphicsPath::changeTool(const DrawTool &newtool) noexcept
{
  ...
  // cache shape
  if (shape_cache.isEmpty()) shape_cache = shape();
}
...
{
  FullGraphicsPath *newpath =
      new FullGraphicsPath(_tool, coordinates.first(), pressures.first());
