 ...
 */
void calcArrowPoints(const QPointF &start, const QPointF &end, QPointF &p1,
                     QPointF &p2)
{
  const qreal length =
      QLineF(start, end).length() / preferences()->arrow_tip_scale;
  const qreal scale1 =
      8. / std::max(40., length) + 32. / std::max(320., length);
  const qreal scale2 = preferences()->arrow_tip_ratio * scale1;
  p1 = {scale2 * start.x() + (1 - scale2) * end.x() -
            scale1 * (end.y() - start.y()),
        scale2 * start.y() + (1 - scale2) * end.y() +
            scale1 * (end.x() - start.x())};
  p2 = {scale2 * start.x() + (1 - scale2) * end.x() +
            scale1 * (end.y() - start.y()),
        scale2 * start.y() + (1 - scale2) * end.y() -
            scale1 * (end.x() - start.x())};
}
...
QList<T *> ArrowGraphicsItem::toPath() const
{
  ...
  if (path().currentPosition().isNull() || rbegin == rend) return {};
  const qreal length = QLineF(rbegin, rend).length();
  const int main_segments = length / 10 + 2, aux_segments = length / 40 + 2;
  QPointF p1, p2;
  calcArrowPoints(rbegin, rend, p1, p2);
  qreal x = rbegin.x(), y = rbegin.y(),
