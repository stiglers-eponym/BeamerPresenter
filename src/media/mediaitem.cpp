...
void MediaItem::createProvider()
{
  ...
  if (_annotation->type() == MediaAnnotation::ExternalURL &&
      _annotation->flags() & MediaAnnotation::IsCaptureSession)
  ...
  switch (_annotation->type()) {
    case MediaAnnotation::ExternalURL:
      _provider->setSource(
          std::static_pointer_cast<ExternalMedia>(_annotation)->url());
      break;
    case MediaAnnotation::EmbeddedFile:
      _provider->setSourceData(
          std::static_pointer_cast<EmbeddedMedia>(_annotation)->data());
      break;
...
std::shared_ptr<MediaItem> MediaItem::fromAnnotation(
