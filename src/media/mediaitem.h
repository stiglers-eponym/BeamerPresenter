...
class MediaItem
{
 ...
 public:
  ...
  /// annotation flags
  int flags() const noexcept { return _annotation->flags(); }

  /// pages on which the video item appears
  const std::set<int> &pages() const noexcept { return _pages; }
  ...
  /// toggle play/pause of media item. Return true of the playing state has
  /// actually changed.
  bool toggle() const { return _provider && _provider->toggle(); }
  ...
  void insertPage(const int page) noexcept { _pages.insert(page); }
  ...
  const QRectF &rect() const noexcept { return _annotation->rect(); }
...
class AudioItem : public QGraphicsRectItem, public MediaItem
{
 ...
 public:
  ...
  AudioItem(std::shared_ptr<MediaAnnotation> &annotation, const int page,
            QGraphicsItem *parent = nullptr)
  ...
  void show() override { QGraphicsRectItem::show(); }

  QGraphicsItem *asQGraphicsItem() noexcept override { return this; }

  virtual int type() const noexcept override { return Type; }
};
...
class VideoItem : public QGraphicsVideoItem, public MediaItem
{
 ...
 public:
  ...
  VideoItem(std::shared_ptr<MediaAnnotation> &annotation, const int page,
            QGraphicsItem *parent = nullptr)
      : QGraphicsVideoItem(parent), MediaItem(annotation, page)
  {
  ...
  void show() override { QGraphicsVideoItem::show(); }
