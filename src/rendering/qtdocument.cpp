...
QtDocument::QtDocument(const QString &filename)
    : PdfDocument(filename), doc(new QPdfDocument())
{
...
#endif
      qCritical() << tr("Could not load document: data not yet available")
                  << path;
      break;
#if (QT_VERSION >= QT_VERSION_CHECK(6, 4, 0))
...
#endif
      qCritical() << tr("Could not load document: unsupported security scheme")
                  << path;
      break;
#if (QT_VERSION >= QT_VERSION_CHECK(6, 4, 0))
                                    ...
                                    const PagePart page_part) const
{
  ...
  const QImage image =
      doc->render(page, (resolution * doc->PAGESIZE_FUNCTION(page)).toSize(),
                  render_options);
  switch (page_part) {
    case LeftHalf:
...
{
  if (flexible_page_sizes >= 0 || !isValid()) return flexible_page_sizes;
...
void QtDocument::loadLabels()
{
  ...
  for (int i = 0; i < doc->pageCount(); i++) {
    ...
    if (label != pageLabels.last()) pageLabels.insert(i, label);
  }
  if (pageLabels.size() == 1 && pageLabels[0] == "") pageLabels.clear();
}
