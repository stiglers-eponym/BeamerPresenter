...
struct ExternalLink : PdfLink {
  QUrl url;
  ExternalLink(const LinkType type, const QRectF &area, const QUrl &url)
      : PdfLink(type, area), url(url)
  {
  }
};
...
struct GotoLink : PdfLink {
  int page;
  GotoLink(const QRectF &area, const int page)
      : PdfLink(PageLink, area), page(page)
  {
  }
};
...
struct ActionLink : PdfLink {
  Action action;
  ActionLink(const QRectF &area, const Action action)
      : PdfLink(LinkType::ActionLink, area), action(action)
  {
  }
};
...
struct MediaLink : PdfLink {
  ...
  MediaLink(const LinkType type, const QRectF &area,
            const std::shared_ptr<MediaAnnotation> annotation)
...
struct PdfOutlineEntry {
  /// Title of the entry.
  QString title;
  /// Page index in the PDF (start counting from 0, destination resolved by the
  /// PDF engine)
  int page;
...
struct SlideTransition {
  ...
  enum Type {
    ...
    Dissolve = 5,
    /// The screen is divided in small squares which change to the next page in
    /// pseudo-random order.
    Glitter = 6,
    ...
    Uncover = 10,
    /// Current slide dissolves, background color becomes visible, and next
    /// slide appears.
    Fade = 11,
...
class PdfDocument
{
 ...
 public:
  ...
  {
    const auto [page, list] = searchAll(needle, start_page, forward);
    return {page, list.isEmpty() ? QRectF() : list.first()};
  }

  /// Search which page contains needle and return the
  /// outline of all occurrences on that slide.
  virtual std::pair<int, QList<QRectF>> searchAll(const QString &needle,
                                                  int start_page = 0,
                                                  bool forward = true) const
  {
    return {-1, {}};
